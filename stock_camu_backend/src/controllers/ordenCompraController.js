...
exports.createOrdenCompra = async (req, res) => {
  try {
    ...
    if (
      ...
      return res.status(400).json({
        message:
          "Datos incompletos. Tipo de lote, tipo de pago, cliente, fecha de emisión y usuario de creación son obligatorios",
      });
    }
    ...
    const ordenCompraConRelaciones = await OrdenCompra.findByPk(
      nuevaOrdenCompra.id,
      {
        include: [
          { model: Cliente, as: "cliente" },
...
exports.updateOrdenCompra = async (req, res) => {
  try {
    ...
    await ordenCompra.update({
      ...
      observacion:
        observacion !== undefined ? observacion : ordenCompra.observacion,
      forma_pago: forma_pago || ordenCompra.forma_pago,
      usuario_modificacion_id:
        usuario_modificacion_id || ordenCompra.usuario_modificacion_id,
...
exports.cambiarEstadoOrdenCompra = async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({
        error: "Error al cambiar estado de la orden de compra",
        details: error.message,
      });
  }
};
...
exports.getOrdenesPendientesPorSocio = async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({
        message: "Error al obtener órdenes pendientes por socio",
        details: error.message,
      });
  }
};
